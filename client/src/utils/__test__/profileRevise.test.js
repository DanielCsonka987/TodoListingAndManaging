...
describe('Login revisions', ()=>{
    ...
    test('Negative - no username', ()=>{
        return loginRev(loginDatas.incorExpls[0]).catch(res=>{
            ...
            expect(res.errorFields[0].field).toBe('username')
            expect(res.errorFields[0].message).toMatch('Username is not acceptable!')
        })
    })
    test('Negative - no password', ()=>{
        return loginRev(loginDatas.incorExpls[1]).catch(res=>{
            ...
            expect(res.name).toBe('MyClientValidateException')
            expect(getType(res.errorFields)).toBe('array')
            expect(res.errorFields[0].field).toBe('password')
            expect(res.errorFields[0].message).toBe('Password is not acceptable!')
        })
    })
...
describe('Register revision', ()=>{
    ...
    test('Negative reg input - password confirmation incorrect', ()=>{
        return regRev(regDatas.incorExpls[2]).catch(res=>{
            expect( getType(res) ).toBe('object')
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('password_repeat')
            expect(res.errorFields[0].message).toBe('Password confirmation is not matching!')
        })
    })
    test('Negative reg input - empty first name', ()=>{
        return regRev(regDatas.incorExpls[3]).catch(res=>{
            expect( getType(res) ).toBe('object')
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('first_name')
            expect(res.errorFields[0].message).toBe('Firstname is not acceptable!')
        })
    })
    test('Negative reg input - age unreasonable', ()=>{
        return regRev(regDatas.incorExpls[4]).catch(res=>{
            expect( getType(res) ).toBe('object')
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('age')
            expect(res.errorFields[0].message).toMatch('as age is not proper!')
        })
    })
...
describe('Password change revision', ()=>{
    ...
    test('Negative input, no old password', ()=>{
        return owdChangeRev(pwdDatas.incorExpls[0]).catch(res=>{
            expect( getType(res) ).toBe('object')
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('old_password')
            expect(res.errorFields[0].message).toMatch('Previous password is not acceptable!')
        })
    })
    test('Negative input, no new password', ()=>{
        return owdChangeRev(pwdDatas.incorExpls[1]).catch(res=>{
            expect( getType(res) ).toBe('object')
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('new_password')
            expect(res.errorFields[0].message).toMatch('New password is not acceptable!')
        })
    })
    test('Negative input, no new password', ()=>{
        return owdChangeRev(pwdDatas.incorExpls[2]).catch(res=>{
            ...
            expect(res.name).toBe('MyClientValidateException')
            expect(res.errorFields[0].field).toBe('password_repeat')
            expect(res.errorFields[0].message).toMatch('New passwords are not matching!')
        })
    })
